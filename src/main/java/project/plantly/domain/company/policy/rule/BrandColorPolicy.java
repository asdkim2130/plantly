package project.plantly.domain.company.policy.rule;

import lombok.RequiredArgsConstructor;
import org.springframework.stereotype.Component;
import project.plantly.domain.company.policy.CompanyMutationPolicy;
import project.plantly.domain.company.policy.CompanyPolicyView;
import project.plantly.domain.company.policy.GradePolicyRegistry;

// 정책(변형): 커스텀 색상이 허용되지 않는 등급은 brandColor 를 기본값(Grey)으로 고정한다. (등록·수정 공통)
// 요청 값을 거부하지 않고 덮어쓴다("고정"). 관리자 등록(ADMIN_EXEMPT)은 면제되므로 제외한다.
// brandColorInScope 가 false 면(수정에서 brandColor 를 안 건드린 경우) 스킵한다 — 무관한 수정에서 색을 리셋하지 않기 위함.
// (등록은 항상 true — 미허용 등급은 색 미입력이어도 기본값으로 고정한다.)
@Component
@RequiredArgsConstructor
public class BrandColorPolicy implements CompanyMutationPolicy {

    // 커스텀 불가 등급의 기본 브랜드 컬러. 표현 형식이 바뀌면 이 값만 조정한다.
    private static final String DEFAULT_BRAND_COLOR = "#808080";

    private final GradePolicyRegistry gradePolicyRegistry;

    @Override
    public void apply(CompanyPolicyView view) {
        if (!view.brandColorInScope() || view.subscription().isExempt()) {
            return;
        }

        if (!gradePolicyRegistry.of(view.subscription().effectiveGrade()).customBrandColorAllowed()) {
            view.company().changeBrandColor(DEFAULT_BRAND_COLOR);
        }
    }
}

...
// '등록 시점' 개념이라 CompanyMutationPolicy 를 달지 않는다 → 수정 경로에서는 재실행되지 않는다.
@Component
...
public class SpotlightPolicy implements CompanyRegistrationPolicy {
    ...
    public void apply(CompanyPolicyView view) {
        ...
        if (gradePolicyRegistry.of(view.subscription().effectiveGrade()).spotlightOnCreate()) {
            view.company().activateSpotlight();
        }
    }
}

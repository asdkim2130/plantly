...
/**
 * 내 회사 목록 읽기 전용 리포지토리. 로그인 유저가 소유(company_member role=OWNER = 본인)한 미삭제 회사를 요약 카드로 나열한다.
 *
...
public class OwnedCompanyCardRepository {
    ...
    // 소유자 본인 + 미삭제. 소유는 company_member(role=OWNER) 로 판정한다.
    // (삭제된 회사는 내 목록에서도 제외 — 소유자 상세조회와 달리 카드 목록은 활성 회사만 노출)
    private static final String WHERE =
            " WHERE EXISTS (SELECT 1 FROM company_member cm" +
            " WHERE cm.company_id = c.id AND cm.user_id = :userId AND cm.role = 'OWNER')" +
            " AND c.deleted = false ";
